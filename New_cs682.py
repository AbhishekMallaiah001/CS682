...
def get_location(zipcode):
    ...
    res = ""
    ip = geocoder.osm(zipcode + ", US")
    if not ip:
        print("Place for the zipcode not found")
        return res
    city = ip.city
    county = ip.county
    if not city and not county:
        return res
    if not ip.city:# case for town
        city = ip.town
    loc = ip.state
    if city:
        loc+= ', '+city 
...
def read_csv_from_date_time(numOfDays):
    ...
    while(currDateCounter <= numOfDays):
        ...
        with open("r.text","w") as f:
            f.write(r.text)
            read_file = pd.read_csv('r.text', encoding='latin-1')
        file_name = 'PVI_'+str(currDateCounter)+'.sv'
...
def main():
    ...
    if(n == 1):
        ...
        if(sys.argv[1] == '-z'):
            ...
            print("Expected either -z as command line arguments")
            return
